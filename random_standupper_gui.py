...
YEAR = 2026
ASSIGNMENTS_FILE = "assignments_2026_gui.json"

# Список всех рабочих дней 2026 года
...
def load_working_days() -> List[datetime.date]:
    """Возвращает отсортированный список рабочих дней."""
    return sorted(datetime.date.fromisoformat(d) for d in WORKING_DAYS_2026)
...
def save_assignments(
    ...
    """Сохраняет распределение докладчиков и настройки в JSON."""
    data = {
        "year": YEAR,
        "names": names,
...
def get_today_speaker(names: List[str], skip_names: List[str]) -> str:
    """Возвращает имя докладчика на сегодня или текст про отдых."""
    today = datetime.date.today()
    working_days = load_working_days()

    if today not in working_days:
        return "сегодня отдыхаем-с!"
    ...
    if stored and stored.get("year") == YEAR:
        ...
        stored_names = stored.get("names", [])
        if stored_skip == skip_set and stored_names == names:
    ...
    if assignments is None:
        ...
        save_assignments(assignments, names, skip_set)

    speaker = assignments.get(today)
    return speaker or "сегодня отдыхаем-с!"


def create_app():
    root = tk.Tk()
    root.title("Random Standupper 2026")

    # Поле с именами
    tk.Label(root, text="Список имён (по одному в строке):").grid(row=0, column=0, sticky="w", padx=5, pady=(5, 0))
    names_text = tk.Text(root, width=30, height=10)
    names_text.grid(row=1, column=0, padx=5, pady=5)
    names_text.insert("1.0", "\n".join(DEFAULT_NAMES))

    # Поле с пропускающими
    tk.Label(root, text="Кого пропустить сегодня (по одному в строке):").grid(
        row=0, column=1, sticky="w", padx=5, pady=(5, 0)
    )
    skip_text = tk.Text(root, width=30, height=10)
    skip_text.grid(row=1, column=1, padx=5, pady=5)
    ...
    def on_calculate():
        raw_names = names_text.get("1.0", "end").strip().splitlines()
        names = [n.strip() for n in raw_names if n.strip()]

        raw_skip = skip_text.get("1.0", "end").strip().splitlines()
        skip = [s.strip() for s in raw_skip if s.strip()]

        if not names:
            messagebox.showerror("Ошибка", "Список имён пуст.")
            return

        try:
            speaker = get_today_speaker(names, skip)
        except ValueError as e:
            messagebox.showerror("Ошибка", str(e))
            return

        result_var.set(speaker)

    btn = tk.Button(root, text="Кто сегодня докладчик?", command=on_calculate)
